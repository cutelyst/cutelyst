...
Cutelee::Node *CSRFTokenTag::getNode(const QString &tagContent, Cutelee::Parser *p) const
{
    Q_UNUSED(tagContent);
    return new CSRFToken(p);
}

CSRFToken::CSRFToken(Cutelee::Parser *parser)
    : Cutelee::Node(parser)
{
}

void CSRFToken::render(Cutelee::OutputStream *stream, Cutelee::Context *gc) const
{
#ifdef PLUGIN_CSRFPROTECTION_ENABLED
    // In case cutelyst context is not set as "c"
    auto c = gc->lookup(m_cutelystContext).value<Cutelyst::Context *>();
    ...
    *stream << QString::fromLatin1(Cutelyst::CSRFProtection::getToken(c));
#else
    Q_UNUSED(stream)
    Q_UNUSED(gc)
    qWarning("%s", "The CSRF protection plugin has not been built.");
