 ...
 *
 * The %Validator plugin provides an API to validate input values that are send by a user to the
 * application, typically in the request body if it is a POST request or the URL query if it is a
 * GET request. The plugin already provides \link plugins-utils-validator-rules validator
 * rules\endlink for common tasks and input types but can be extended by deriving a new rule class
 * from ValidatorRule. %Validator rules are not meant to be used on their own but as part of the
 * Validator class that processes the input data. However some validator rules export their
 * validation logic in a static member function that can be used without Validator directly on a
 * value. See the documentation of Validator to learn more about how to use it.
 *
 * <h3>Logging</h3>
 * Information is logged to the \c cutelyst.utils.validator logging category. Failed validation will
 * only be logged if debug output is enabled. Other errors like failed parsing and missing
 * validation data will be logged to the warning category.
 *
 * <h3>Building and using</h3>
 * The plugin is linked to %Cutelyst Core API and the QtNetwork module. To use it in your
 * application, link your application to \a Cutelyst::Utils::Validator.
 *
 * <h4>Optional validators</h4>
 * ValidatorPwQuality relies on <a
 * href="https://github.com/libpwquality/libpwquality">libpwquality</a> and will not be included and
 * build by default. Use either <code>-DPLUGIN_VALIDATOR_PWQUALITY:BOOL=ON</code> or
...
public:
     ...
     * ValidatorResult that contains information about validation errors, if any. The result can be
     * checked directly in \c if statements.
     *
     * If Validator::FillStashOnError is set, it will fill the stash of Context \c with error data
     * and not sensible input values.
     */
     ...
     * about validation errors, if any. The result can be checked directly in \c if statements.
     *
     * If Validator::FillStashOnError is set, it will fill the stash of Context \c with error data
     * and not sensible input values.
     */
