 ...
 * given, the validator will at first try to parse the date according to that format. If that fails
 * or if there is no custom \a inputFormat set, it will try to parse the date and time based on
 * standard formats in the following order: \link Context::locale() Context locale's \endlink \link
 * QLocale::toDate() toDateTime() \endlink with QLocale::ShortFormat and QLocale::LongFormat,
...
public:
     ...
     */
    ValidatorDateTime(const QString &field,
