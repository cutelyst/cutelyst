 ...
 * either binary (MiB, etc.) or decimal (MB, etc.) file sizes. Negative values are not allowed.
 * White space (space and tab) is allowed and will be ignored by the validation logic. Size prefixes
 * from kB to YiB are supported.
 *
 * This validation rule is locale aware, so be sure to set the correct \link Context::locale()
 * locale on your context\endlink. The locale awareness is especially important to select the
 * correct decimal separator sign. To support both language directions, the validation logic
 * supports both positions of the prefix symbol for all languages (e.g. "KB 2" and "2 KB" are
 * handled the same way and are both valid).
 *
 * Unless a specific \link ValidatorFileSize::Option option\endlink has been selected, the validator
 * will use binary or decimal base according to the prefix. (2KiB will return 2048, 2kB will return
 * 2000). Recognizion of the prefix is case insensitive.
 *
 * \note The conversion of big sizes like Petabyte and above might lead to rounding issues in the
 * return value.
 *
...
public:
    ...
    enum Option : quint8 {
        ...
        ForceBinary = 3, /**< Forces the usage of the binary system when generating the return value
                            but will not fail if the prefix symbol is decimal. */
        ForceDecimal = 4 /**< Forces the usage of the decimal system when generating the return
     ...
     */
    static void inputPattern(Context *c,
