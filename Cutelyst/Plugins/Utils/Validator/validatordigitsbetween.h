 ...
 * \class ValidatorDigitsBetween validatordigitsbetween.h
 * <Cutelyst/Plugins/Utils/validatordigitsbetween.h> \brief Checks for digits only with a length
 * between min and max.
 *
...
public:
     ...
     * \param min           Minimum length of the digits. Should either be an integer value to
     * directly specify the length or the name of a \link Context::stash() Stash\endlink key
     * containing the length constraint. \param max           Maximum length of the digits. Should
     * either be an integer value to directly specify the length or the name of a \link
     * Context::stash() Stash\endlink key containing the length constraint. \param messages Custom
     * error messages if validation fails. \param defValKey     \link Context::stash() Stash
     * \endlink key containing a default value if input field is empty. This value will \b NOT be
     * validated.
     */
     ...
     * \a max. \param value The value to validate as it is. \param min   Minimum length of the
     * digits. \param max   Maximum length of the digits. \return \c true if \a value string only
     * contains digits and has a length between \a min and \a max, otherwise it returns \a false.
...
protected:
     ...
     *
     * \a errorData will contain a QVariantList with the \a min value as first and the \a max value
     * as second entry.
     */
