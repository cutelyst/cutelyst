...
#include <Cutelyst/Plugins/csrfprotection_export.h>
 ...
 *
 * The CSRFProtection plugin implements a synchronizer token pattern (STP) to protect input forms
 * against <A HREF="https://en.wikipedia.org/wiki/Cross-site_request_forgery">Cross Site %Request
 * Forgery (CSRF/XSRF) attacks</A>. This type of attack occurs when a malicious website contains a
 * link, a form button or some JavaScript that is intended to perform some action on your website,
 * using the credentials of a logged-in user who visits the malicious site in their browser.
 *
 * The first defense against CSRF attacks is to ensure that GET requests (and other @a safe methods,
 * as defined by <A HREF="https://tools.ietf.org/html/rfc7231.html#section-4.2.1">RFC 7231</A>) are
 * side effect free. Requests via @a unsafe methods, such as POST, PUT, and DELETE, can then be
 * protected by this plugin.
 *
 * This plugin has been inspired by the <A
 * HREf="https://docs.djangoproject.com/en/1.11/ref/csrf/">CSRF protection of the Django web
 * framework</A>.
 *
 * <H3>Usage</H3>
 *
 ...
 * the protection for single namespace members by adding <CODE>:CSRFRequire</CODE> to the
 * attributes.
 *
 * @code{.cpp}
 ...
 *
 * If the CSRF protection check fails, the return code will be set to 403 - Forbidden and an error
 * message will be set to the stash key defined by setErrorMsgStashKey(). You can set a default
 ...
 *
 * If you are using ajax to submit form requests or if you use AJAX without a HTML form, you have to
 * provide the CSRF token too. If you are using the normal way by setting a cookie, you can read the
 * CSRF token from that cookie. If you use the session to store the token, you have to include the
 * token somewhere into the DOM tree from where you can read it. You can then add the extracted
 * token to the POST data of every POST request or you can can set a custom X-CSRFToken header to
 * the value of the CSRF token. The latter method is often easier, because many JavaScript
 * frameworks provide hooks that allow headers to be set on every request.
 *
 * <H3>How it works</H3>
 *
 * On every request, a secret token is set that is stored in a cookie or in the user session and has
 * to be send back to the application when performing actions on unsafe methods like POST. The token
 * stored in the cookie or in the session is salted with another random value. The same secret with
 ...
 * setting the cookie and using a corresponding token, subdomains will be able to circumvent the
 * CSRF protection. The only way to avoid this is to ensure that subdomains are controlled by
 * trusted users (or, are at least unable to set cookies). Note that even without CSRF, there are
 ...
 * persistent cookies, the form submission would fail in this case.
 *
 * Some browsers (specifically Internet Explorer) can disallow the use of persistent cookies or can
 ...
 * from @c subdomain.example.com from succeeding against @c api.example.com. If you need
...
public:
     ...
     * no detaching and a @link setGenericErrorMessage() generic error@endlink page will be
     * generated.
     */
     ...
     * have single methods in your namespaces that should still be protected, use the
     * <CODE>:CSRFRequire</CODE> attribute on this methods.
     */
    ...
    /**
     * Whether to use HttpOnly flag on the CSRF cookie. If this is set to @c true, client-side
     * JavaScript will not to be able to access the CSRF cookie. The default is @c false.
     *
     * Designating the CSRF cookie as @c HttpOnly doesn’t offer any practical protection because
     * CSRF is only to protect against cross-domain attacks. If an attacker can read the cookie via
     * JavaScript, they’re already on the same domain as far as the browser knows, so they can do
     ...
     * auditors.
     *
     * If you enable this and need to send the value of the CSRF token with an AJAX request, your
     * JavaScript must pull the value from a hidden CSRF token form input on the page instead of
     * from the cookie.
     */
    void setCookieHttpOnly(bool httpOnly);

    /**
     * The name of the cookie to use for the CSRF authentication token. The default is @a
     ...
     * @c true. For all other methods it will return @c false if the CSRF protection check has
     * failed.
     */
