 ...
 * handled. If the comparative value is a QDateTime, the input data will be converted into a
 ...
 * first tried to create a valid QTimeZone from the \a timeZone, if that fails it will first try to
 * get the time zone from the input parameters and if there is no key with that name, trying it with
 ...
 * for every language new ValidatorAfter(QStringLiteral("datefield2"),
 *                                     QDateTime::fromString(QStringLiteral("2018-01-01T18:18:18"),
 * Qt::ISODate), QString(), QT_TRANSLATE_NOOP("MyController", "MM/dd/yyyy, HH:mm")),
 *
 ...
 * field new ValidatorAfter(QStringLiteral("datetime3"), QDateTime(QDate(2018, 1, 15), QTime(12,0),
