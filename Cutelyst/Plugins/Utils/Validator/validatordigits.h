 ...
 * meant to test for an exact numeric value but for the string length.
 *
...
public:
     ...
     * disables the length check. Should be either an int to directly specify the length or the name
     * of an input field or \link Context::stash() Stash \endlink key containing the length
     * constraint. \param messages      Custom error messages if validation fails. \param defValKey
     ...
     */
    ValidatorDigits(const QString &field,
...
protected:
     ...
     *
     * \a errorData will contain \c 0, if \a length is greater than \c 0 and does not match the
     * field value length, \a errorData will contain \c 1.
     */
