...
void TestPbkdf2::doTest()
{
    ...
    const QByteArray ret =
        CredentialPassword::pbkdf2(QCryptographicHash::Sha1, password, salt, count, keyLength);
...
void TestPbkdf2::testPbkdf2_data()
{
    ...
    QTest::newRow("pbkdf2-test00")
        << QByteArrayLiteral("password") << QByteArrayLiteral("salt") << 1 << 20
        << QByteArrayLiteral("0c60c80f961f0e71"
                             "f3a9b524af601206"
                             "2fe037a6");

    QTest::newRow("pbkdf2-test01")
        << QByteArrayLiteral("password") << QByteArrayLiteral("salt") << 2 << 20
        << QByteArrayLiteral("ea6c014dc72d6f8c"
                             "cd1ed92ace1d41f0"
                             "d8de8957");

    QTest::newRow("pbkdf1-test02")
        << QByteArrayLiteral("password") << QByteArrayLiteral("salt") << 4096 << 20
        << QByteArrayLiteral("4b007901b765489a"
                             "bead49d926f721d0"
                             "65a429c1");

    //    QTest::newRow("pbkdf2-test03") << QByteArrayLiteral("password") <<
    //    QByteArrayLiteral("salt") << 16777216 << 20
    //                                   << QByteArrayLiteral("eefe3d61cd4da4e4"
    //                                                        "e9945b3d6ba2158c"
    //                                                        "2634e984");

    QTest::newRow("pbkdf2-test04")
        << QByteArrayLiteral("passwordPASSWORDpassword")
        << QByteArrayLiteral("saltSALTsaltSALTsaltSALTsaltSALTsalt") << 4096 << 25
        << QByteArrayLiteral("3d2eec4fe41c849b"
                             "80c8d83662c0e44a"
                             "8b291a964cf2f070"
                             "38");

    QTest::newRow("pbkdf2-test05")
