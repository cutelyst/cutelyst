 ...
 * \class ValidatorRequiredUnless validatorrequiredunless.h
 * <Cutelyst/Plugins/Utils/validatorrequiredunless.h> \brief The field under validation must be
 * present and not empty unless the other field is equal to any value in the list.
 *
...
public:
     ...
     * \param otherValues   List of values that are not allowed to be in the other field to require
     * the main field. \param messages      Custom error messages if validation fails.
     */
...
private:
    Q_DECLARE_PRIVATE(ValidatorRequiredUnless) // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
